...
/**
 * Created by liubin on 2015/12/26.
 */
public class MobileCart extends BaseEntity {
    ...
    /** 购物车项 */
    private Set<MobileCartItem> mobileCartItems = new HashSet<MobileCartItem>();
    ...
    public int getWeight() {
        int weight = 0;
        if (getMobileCartItems() != null) {
            for (MobileCartItem cartItem : getMobileCartItems()) {
                if (cartItem != null) {
    ...
    public long getAddedPoint() {
        ...
        for (Promotion promotion : getPromotions()) {
            if (promotion != null) {
                ...
                Set<MobileCartItem> MobileCartItems = getMobileCartItems(promotion);
                for (MobileCartItem cartItem : MobileCartItems) {
    ...
    public BigDecimal getDiscount() {
        ...
        for (Promotion promotion : getPromotions()) {
            if (promotion != null) {
                ...
                Set<MobileCartItem> cartItems = getMobileCartItems(promotion);
                for (MobileCartItem cartItem : cartItems) {
     ...
     */
    @Transient
    private Set<MobileCartItem> getMobileCartItems(Promotion promotion) {
        Set<MobileCartItem> cartItems = new HashSet<MobileCartItem>();
        if (promotion != null && getMobileCartItems() != null) {
            for (MobileCartItem cartItem : getMobileCartItems()) {
    ...
    private int getQuantity(Promotion promotion) {
        int quantity = 0;
        for (MobileCartItem cartItem : getMobileCartItems(promotion)) {
            if (cartItem != null && cartItem.getQuantity() != null) {
                quantity += cartItem.getQuantity();
            }
        }
     ...
     */
    @Transient
    public MobileCartItem getCartItem(Product product) {
        if (product != null && getMobileCartItems() != null) {
            for (MobileCartItem cartItem : getMobileCartItems()) {
    ...
    public boolean isEmpty() {
        return getMobileCartItems() == null || getMobileCartItems().isEmpty();
    }

    public Set<MobileCartItem> getMobileCartItems() {
        return mobileCartItems;
    }

    public void setMobileCartItems(Set<MobileCartItem> mobileCartItems) {
