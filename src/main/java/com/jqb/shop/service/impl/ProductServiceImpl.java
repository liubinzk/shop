...
public class ProductServiceImpl extends BaseServiceImpl<Product, Long> implements ProductService, DisposableBean {
	...
	@Transactional(readOnly = true)
	public Page<Product> findPage(Area area, ProductCategory productCategory, Brand brand, Promotion promotion, List<Tag> tags, Map<Attribute, String> attributeValue, BigDecimal startPrice, BigDecimal endPrice, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isGift, Boolean isOutOfStock, Boolean isStockAlert, OrderType orderType, Pageable pageable) {
		return productDao.findPage(area,productCategory, brand, promotion, tags, attributeValue, startPrice, endPrice, isMarketable, isList, isTop, isGift, isOutOfStock, isStockAlert, orderType, pageable);
	}
	...
	checkProduct(Member member, Product product){
		...
		if(product != null ){
			...
			filter=new Filter("sn", Filter.Operator.eq,product.getSn());
			...
			if(restrictNum >= product.getRestrictionNum() ){
//				String result = product.getFullName() + "已超过限购数量，不能购买！";
				String result = "此商品限购"+product.getRestrictionNum()+"份，已达到限购数量，不能购买！";
