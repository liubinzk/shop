...
public class ProductCategoryController extends BaseController {
	...
	@Resource(name = "productCategoryImageServiceImpl")
	private ProductCategoryImageService productCategoryImageService;
	...
	public String save(ProductCategory productCategory, Long parentId, Long[] brandIds, RedirectAttributes redirectAttributes) {
		...
		for (Iterator<ProductCategoryImage> iterator = productCategory.getProductCategoryImages().iterator(); iterator.hasNext();) {
			ProductCategoryImage productCategoryImage = iterator.next();
			if (productCategoryImage == null || productCategoryImage.isEmpty()) {
				iterator.remove();
				continue;
			}
			if (productCategoryImage.getFile() != null && !productCategoryImage.getFile().isEmpty()) {
				if (!fileService.isValid(FileInfo.FileType.image, productCategoryImage.getFile())) {
					addFlashMessage(redirectAttributes, Message.error("admin.upload.invalid"));
					return "redirect:edit.jhtml?id=" + productCategory.getId();
				}
			}
		}

		for (ProductCategoryImage productCategoryImage : productCategory.getProductCategoryImages()) {
			productCategoryImageService.build(productCategoryImage);
		}
		Collections.sort(productCategory.getProductCategoryImages());
