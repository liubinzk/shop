...
public class  MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	public Member findByUid(long uid) {
		try {
			String jpql = "select members from Member members where members.uid = :uid";
			return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("uid", uid).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
	}
	public long getMemberId(long uid) {
		try {
			String jpql = "select members.id from Member members where members.uid = :uid";
			return entityManager.createQuery(jpql, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("uid", uid).getSingleResult();
		} catch (NoResultException e) {
			return -1;
		}
	}
