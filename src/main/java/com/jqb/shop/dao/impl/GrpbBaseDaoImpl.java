 ...
 * @version 3.0
 */
public abstract class GrpbBaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	protected Page<T> findPage(CriteriaQuery<T> criteriaQuery, Pageable pageable) {
		...
		if (totalPages < pageable.getPageNumber()) {
			...
			return new Page<T>(new ArrayList<T>(), total, pageable);
		}
		TypedQuery<T> query = entityManager.createQuery(criteriaQuery).setFlushMode(FlushModeType.COMMIT);
	...
	private void addRestrictions(CriteriaQuery<T> criteriaQuery, Pageable pageable) {
		...
		if (StringUtils.isNotEmpty(pageable.getSearchProperty()) && StringUtils.isNotEmpty(pageable.getSearchValue())) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.like(processsPageableFilter(root,pageable.getSearchProperty()), "%" + pageable.getSearchValue() + "%"));
		}
		if (pageable.getFilters() != null) {
			for (Filter filter : pageable.getFilters()) {
				...
				if (filter.getOperator() == Operator.eq && filter.getValue() != null) {
					if (filter.getIgnoreCase() != null && filter.getIgnoreCase() && filter.getValue() instanceof String) {
						restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(criteriaBuilder.lower(processsPageableFilter(root,filter.getProperty())), ((String) filter.getValue()).toLowerCase()));
					} else {
						restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(processsPageableFilter(root,filter.getProperty()), filter.getValue()));
					}
				} else if (filter.getOperator() == Operator.ne && filter.getValue() != null) {
					if (filter.getIgnoreCase() != null && filter.getIgnoreCase() && filter.getValue() instanceof String) {
						restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.notEqual(criteriaBuilder.lower(root.<String> get(filter.getProperty())), ((String) filter.getValue()).toLowerCase()));
					...
					restrictions = criteriaBuilder.and(restrictions, root.get(filter.getProperty()).isNotNull());
				} else if (filter.getOperator() == Operator.greaterThan) {
				...
				}  else if (filter.getOperator() == Operator.lessThan) {
					restrictions =  criteriaBuilder.and( criteriaBuilder.lessThan(root.<Date>get(filter.getProperty()), (Date) filter.getValue()) );
				}
	...
	private Path processsPageableFilter(Root root,String property){
		String[] ps=property.split("\\.");
		Path path=root.get(ps[0]);
