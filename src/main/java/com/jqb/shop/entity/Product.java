...
import com.jqb.shop.BigDecimalNumericFieldBridge;
...
public class Product extends BaseEntity  {
	...
	@Transient
	private List<Product> siblings = new ArrayList<Product>();

	static {
		try {
			File jqbShopXmlFile = new ClassPathResource(CommonAttributes.JQBSHOP_XML_PATH).getFile();
			org.dom4j.Document document = new SAXReader().read(jqbShopXmlFile);
	 ...
	 */
	@Transient
	public List<Product> getProductSiblings() {
		...
		if (getGoods() != null && getGoods().getProducts() != null) {
			for (Product product : getGoods().getProducts()) {
				if (!this.equals(product)) {
					...
					selfProduct.setId(-this.getId());
					selfProduct.setSn(this.getSn());
					selfProduct.setFullName(this.fullName);
					selfProduct.initRestEntiy();
					siblings.add(selfProduct);
				}
			}
		}
		return siblings;
	...
	public void setSiblings(List<Product> siblings) {
	...
	public void setIntroductionImg(List<String> introductionImg) {
		this.introductionImg = introductionImg;
	}

	@Column(name="is_pay_shipping")
	public Boolean getIsPayShipping() {
		return isPayShipping;
	}

	public void setIsPayShipping(Boolean isPayShipping) {
		this.isPayShipping = isPayShipping;
	}

	@FieldBridge(impl = BigDecimalNumericFieldBridge.class)
	@Min(0)
	@Digits(integer = 12, fraction = 3)
	@Column(name="shipping_price", precision = 21, scale = 6)
	...
	public void countAmount(){
		...
		if (isPayShipping) {
			amount = amount.add( getShippingPrice()  );
		}
	...
	public void initRestEntiy(){
		this.setCartItems(null);
		this.setSpecifications(null);
		this.setPromotions(null);
		this.setTags(null);
		this.setSpecificationValues(null);
		this.setReviews(null);
		this.setProductNotifies(null);
		this.setConsultations(null);
		this.setBrand(null);
		this.setFavoriteMembers(null);
		this.setGiftItems(null);
		this.setGoods(null);
		this.setMemberPrice(null);
