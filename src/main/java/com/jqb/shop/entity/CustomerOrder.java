 ...
 */
@Entity
@Table(name = "xx_customer_order")
@SequenceGenerator(name = "sequenceGenerator", sequenceName = "xx_customer_order_sequence")
public class CustomerOrder extends BaseEntity {
	...
	/** 订单项 */
	private List<Order> realOrders = new ArrayList<Order>();

	/** 订单日志 */
	private Set<CustomerOrderLog> orderLogs = new HashSet<CustomerOrderLog>();
	...
	public void setRealOrders(List<Order> realOrders) {
	 ...
	 * @return 订单日志
	 */
	@OneToMany(mappedBy = "customerOrder", fetch = FetchType.LAZY, cascade = CascadeType.REMOVE)
	@OrderBy("createDate asc")
	public Set<CustomerOrderLog> getOrderLogs() {
		return orderLogs;
	}

	public void setOrderLogs(Set<CustomerOrderLog> orderLogs) {
	...
	public String getName() {
		...
		if (getRealOrders() != null) {
			for (Order order : getRealOrders() ){
				name.append(NAME_SEPARATOR).append( order.getName() );
			}
			if (name.length() > 0) {
				name.deleteCharAt(0);
			}
	...
	public int getWeight() {
		int weight = 0;
		if (getRealOrders() != null) {
			for (Order orderItem : getRealOrders()) {
				if (orderItem != null) {
	...
	public BigDecimal getPrice() {
		...
		if (getRealOrders() != null) {
			for (Order realOrder : getRealOrders()) {
				if (realOrder != null ) {
					price = price.add(realOrder.getPrice());
				}
			}
		}
		return price;
	...
	public OrderItem getOrderItem(String sn) {
		if (sn != null && getRealOrders() != null) {
			for(Order realOrder :  getRealOrders()){
				for (OrderItem orderItem : realOrder.getOrderItems()) {
	...
	public void initRestEntity() {
		this.paymentMethod = null;
		this.shippingMethod = null;
		this.operator = null;
		this.orderLogs = null;
	}
	public void initRestEntityButReturns() {
		//this.paymentMethod = null;
		this.shippingMethod = null;
		this.operator = null;
		this.orderLogs = null;
