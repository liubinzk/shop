...
<%@page import="com.jqb.shop.CommonAttributes"%>
<%@page import="com.jqb.shop.util.JsonUtils"%>
...
<%
	...
	if (status.equals("success")) {
		...
		try {
			Resource resource = new ClassPathResource(CommonAttributes.JQBSHOP_PROPERTIES_PATH);
			...
			properties.store(outputStream, "JQB SHOP PROPERTIES");
			outputStream.close();
		} catch (IOException e) {
			status = "error";
			message = "JQBSHOP.PROPERTIES文件写入失败!";
			exception = stackToString(e);
		}
	}
	...
	if (status.equals("success")) {
		try {
			File shopxxXmlFile = new ClassPathResource(CommonAttributes.JQBSHOP_XML_PATH).getFile();
			Document document = new SAXReader().read(shopxxXmlFile);
			Element siteUrlElement = (Element) document.selectSingleNode("/shop/setting[@name='siteUrl']");
			Element logoElement = (Element) document.selectSingleNode("/shop/setting[@name='logo']");
			Element defaultLargeProductImageElement = (Element) document.selectSingleNode("/shop/setting[@name='defaultLargeProductImage']");
			Element defaultMediumProductImageElement = (Element) document.selectSingleNode("/shop/setting[@name='defaultMediumProductImage']");
			Element defaultThumbnailProductImageElement = (Element) document.selectSingleNode("/shop/setting[@name='defaultThumbnailProductImage']");
			String siteUrl;
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_LOCK_CONFIG_PATH), "JQB SHOP INSTALL LOCK - SHOPXX.NET");
		} catch (IOException e) {
			status = "error";
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_INIT_CONFIG_PATH), "JQB SHOP INSTALL INIT - SHOPXX.NET");
